...
export default function BookingsClient() {
  ...
  useEffect(() => {
    setLoading(true)
    const url = status === 'all' ? '/api/admin/bookings' : `/api/admin/bookings?status=${status}`
    fetch(url)
      .then(r => r.json())
      .then((data) => {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <p className="text-muted-foreground mb-6 text-sm">
        {loading ? '…' : `${bookings.length} request${bookings.length !== 1 ? 's' : ''}`}
      </p>
      ...
      {loading ? (
        <p className="text-muted-foreground text-sm py-12 text-center border border-border rounded-sm">Loading…</p>
      ) : !bookings.length ? (
