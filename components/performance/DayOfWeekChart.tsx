...
import { calcDayOfWeekPerformance } from '@/lib/analytics/benchmark';
import type { DayStats } from '@/lib/analytics/benchmark';
...
export function DayOfWeekChart({ dailyValues }: DayOfWeekChartProps) {
  const stats: DayStats[] = useMemo(() => {
    if (dailyValues.length < 20) return [];
    return calcDayOfWeekPerformance(dailyValues);
  }, [dailyValues]);

  const handleExportCSV = useCallback(() => {
    if (stats.length === 0) return;
    const header = 'Day,Avg Return,Median Return,Win Rate,Std Dev,N';
    ...
    a.download = 'day-of-week-performance.csv';
    a.click();
        ...
        <table className="w-full text-[11px] font-mono">
          <thead>
            <tr className="border-b border-[var(--border-subtle)]">
              {['Day', 'Avg Return', 'Median', 'Win Rate', 'Std Dev', 'n'].map((h) => (
                <th
                  key={h}
          ...
          <tbody>
            {stats.map((s) => (
              <tr key={s.day} className="border-b border-[var(--border-subtle)]">
                <td className="py-1.5 pr-4 text-[var(--text-secondary)]">{s.day}</td>
                <td
