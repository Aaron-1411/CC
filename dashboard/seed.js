  ...
  var MEMORY = [
    {
      ...
      sections: {
        ...
        decisions: [
          ...
          {
            id: 'd-7', date: '2026-06-12', title: 'Phase V — production store stays on localStorage; durable workspace is ~/cc-hub',
            context: 'MASTER-BUILD v2 added a Verification Gate (Phase V) before further build-out, and the STORAGE MAP three-homes boundary (cc-knowledge / Supabase / dashboard). Part A is on-disk consolidation; Part B needs deployed-runtime checks (live Anthropic, Cloudflare env vars) that require access not yet granted.',
            decision: 'Run Phase V Part A only. Keep the production store adapter on localStorage (config.js storeAdapter:"local") as an accepted decision — Supabase stays opt-in and reversible, NOT "supabase confirmed". Treat ~/cc-hub (clean CC clone) as the durable on-disk workspace, overriding the doc\'s ~/Claude-Projects assumption (that checkout carries uncommitted client work + a launchd dependency and is never touched). Document the STORAGE MAP as ROADMAP Appendix A and the knowledge-repo rules as CLAUDE.md Appendix B.',
            consequences: 'Part A landed in small commits and passed a fresh-context subagent review. Runtime VERIFY items (deployed Chairman → audit_log row + token capture; Cloudflare confirmation of ANTHROPIC_API_KEY / SUPABASE_URL / SUPABASE_ANON_KEY) remain blocked pending access, so `phase-v-complete` is intentionally not tagged. The daily spend indicator (locked decision #7) is deferred to Phase 7a. Stale clones (~/Claude-Projects, /tmp/CC-fresh) flagged for owner cleanup in ROADMAP-NOTES.',
          },
        ],
      ...
      sessions: [
        ...
        {
          id: 's-7', date: '2026-06-12', title: 'Phase V — Verification Gate (Part A consolidation)',
          summary: '## Phase V Part A (consolidation)\n\nRan the MASTER-BUILD v2 Verification Gate on-disk consolidation against the durable workspace `~/cc-hub` (clean CC clone — overrides the doc\'s `~/Claude-Projects` assumption, which carries uncommitted client work + a launchd dependency and is never touched). Landed in small commits, each one logical change: appended the **STORAGE MAP** as ROADMAP Appendix A (three homes — cc-knowledge for skills/agents/memory, Supabase for the operations floor, dashboard/ as the machine; "knowledge never in Supabase, operations never in git"); expanded CLAUDE.md with the 10-step session protocol + knowledge-repo rules (Appendix B); created `BUILD-STATE.md` (12-phase tracker, Phase V in progress); reconciled `ARCHITECTURE.md` to the live app (workspace.html ~2340 lines, eight views incl. Chairman, HubStore backs the renderers, localStorage adapter in prod, Pages Functions exist); recorded Phase V notes + spillover in ROADMAP-NOTES; and untracked the machine-specific `.claude/launch.json` via `git rm --cached` (it was already committed in the clone, so the `.gitignore` rule had been a no-op). **Production store stays on localStorage** as an accepted decision (Supabase opt-in + reversible, NOT "supabase confirmed"). Fresh-context subagent review returned PASS. Runtime Part B VERIFY items (deployed Chairman → audit_log row + token capture; Cloudflare env-var confirmation) remain **blocked** pending access, so `phase-v-complete` is intentionally **not** tagged. Daily spend indicator deferred to Phase 7a.',
          tokensEstimate: 30000,
        },
      ],
    },
  ];
