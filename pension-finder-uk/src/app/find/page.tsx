...
import { Plus, Trash2, ChevronRight, ChevronLeft, ExternalLink, CheckCircle, Building2 } from "lucide-react";
      ...
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} className="flex items-center gap-2">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
            i < current ? "bg-indigo-700 text-white" : i === current ? "bg-indigo-100 text-indigo-700 ring-2 ring-indigo-700" : "bg-gray-100 text-gray-400"
          }`}>
    ...
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="text"
            placeholder="e.g. 2015 or current"
            value={job.endYear}
...
function getPensionLikelihood(job: Job): { label: string; color: string; notes: string[] } {
  ...
  if (years < 2) notes.push("Short tenure — you may have been opted out or refunded contributions.");
  ...
  return (
    <div className="flex flex-col gap-8">
          ...
          <div key={i} className={`border rounded-xl p-5 ${colorMap[color]}`}>
            <div className="flex items-start justify-between gap-4 mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900">{job.employer || "Unnamed employer"}</h3>
                  <span className="text-xs text-gray-500">{job.startYear}–{job.endYear} · {job.industry || "Unknown sector"}</span>
                </div>
              </div>
              <span className={`text-xs font-bold px-2 py-1 rounded-full ${badgeMap[color]}`}>
                {label} likelihood
              </span>
            </div>
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-5">
        ...
        <p className="text-sm text-gray-600 mb-3">Once you&apos;ve located your pots, add them to your tracker to see the full picture and run a projection.</p>
